...
// S3 公開路徑範本
const S3_BASE =
  "https://s3.us-west-2.amazonaws.com/generate.imagenfile/uploads";
...
export default function AIDesignPage() {
  ...
  const [fileIds, setFileIds] = useState({});         // 上傳後回傳的檔案 ID
  ...
  // 處理檔案上傳：上傳到 S3，並把回傳的 ID 存到 fileIds
  ...
  const handleGenerate = async (customPrompt) => {
    const usedPrompt = customPrompt != null ? customPrompt : prompt;
    ...
    try {
      ...
      const { imageUrl, bestPrompt: generatedBestPrompt } =
        await res.json();
    ...
    <div className="h-screen grid grid-cols-[3fr_2fr_1fr] gap-6 bg-white p-6 text-gray-800 overflow-hidden">
      ...
      <div className="flex flex-col border-2 border-orange-500 bg-gray-50 rounded-2xl p-4 overflow-hidden">
        ...
        <div className="flex justify-around mt-4">
          ...
          <button
            className="w-40 h-12 flex items-center justify-center gap-2 bg-orange-500 text-white rounded-lg text-lg"
            onClick={() => setImageURL("")}
          >
            <Download size={20} /> 下載
          </button>
      ...
      <div className="flex flex-col border-2 border-blue-500 bg-gray-50 rounded-2xl p-4 overflow-hidden">
        <div className="p-3 bg-blue-500 text-white rounded-lg">
          ...
          {bestPrompt && (
            <p className="mt-2 whitespace-pre-wrap">{bestPrompt}</p>
          )}
        </div>
                ...
                <div key={idx} className="flex items-center gap-4 w-full">
                  ...
                  <div className="flex flex-col items-center w-full">
                    {previewSrc ? (
                      ...
                      <div className="w-full max-w-[200px] aspect-square bg-gray-200 rounded animate-pulse" />
                    )}
                    <label
                      htmlFor={`file-${idx}`}
